...
class UserDTOToEntityConverter(
    DTOToEntityConverter[UserRegistrationRequestDTO, UserEntity], LoggingMixin
):
    ...
    def to_entity(
        ...
        """
        self.logger.debug(
            f"Converting UserRegistrationRequestDTO to UserEntity for email: {dto.email}"
        )
        ...
        try:
            ...
            self.logger.debug(
                "Successfully converted UserRegistrationRequestDTO to UserEntity"
            )
            return entity
...
class UserBidirectionalConverter(
    BidirectionalConverter[UserEntity, UserResponseDTO], LoggingMixin
):
    ...
    def to_dto(self, entity: UserEntity) -> UserResponseDTO:
        ...
        """
        self.logger.debug(
            f"Converting UserEntity to UserResponseDTO (bidirectional) for user: {entity.user_id}"
        )
    ...
    def to_entity(self, dto: UserResponseDTO) -> UserEntity:
        ...
        """
        self.logger.debug(
            f"Converting UserResponseDTO to UserEntity for user: {dto.user_id}"
        )
