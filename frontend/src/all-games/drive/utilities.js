...
function clampAngle(value) {
  return ((((value + PI) % (2 * PI)) + 2 * PI) % (2 * PI)) - PI;
}
function percent(value, valueA, valueB) {
  const d = valueB - valueA;
  return d ? clamp((value - valueA) / d) : 0;
}
...
class Vector3 {
  ...
  max(v) {
    ASSERT_VEC3(v);
    return vec3(max(this.x, v.x), max(this.y, v.y), max(this.z, v.z));
  }
  min(v) {
    ASSERT_VEC3(v);
    return vec3(min(this.x, v.x), min(this.y, v.y), min(this.z, v.z));
  }
  floor() {
    return vec3(floor(this.x), floor(this.y), floor(this.z));
  }
  round() {
  ...
  DOMPoint() {
    return new DOMPoint(this.x, this.y, this.z);
  }
  rotateX(a) {
    const c = Math.cos(a),
      s = Math.sin(a);
