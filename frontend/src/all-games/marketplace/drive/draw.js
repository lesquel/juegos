...
class Mesh {
  ...
  buildExtrude(facePoints, size = 1, topScale = 1) {
    ...
    for (let k = 2; k--; )
      for (let i = vertCount; i--; ) {
        ...
        const point = facePoints[
          j % 2 == (vertCount % 2 ? 0 : k) ? vertCount - 3 - h : h
        ].scale(k ? topScale : 1);
        point.z = k ? size : -size;
        points.push(point);
    ...
    for (let i = facePoints.length; i--; ) {
      ...
      for (const p of sidePoints) normals.push(normal);
    }
  ...
  renderUnlit(transform, color, unlit = true) {
    if (unlit) glPushMonoColoredPoints(this.points, color);
    else glPushPoints(this.points, this.normals, color);
    glRender(transform);
  }
...
function pushGradient(pos, size, color, color2) {
  ...
  const points = mesh.points.map((p) =>
    vec3(p.x * size.x + pos.x, p.y * size.y + pos.y, p.z * size.z + pos.z)
  );

  let colors = [];
  for (let i = 0; i < points.length; i++) colors[i] = i > 2 ? color2 : color;
...
function pushSprite(pos, size, color, tile) {
  ...
  } else glPushPoints(points, 0, color);
}

function pushShadow(pos, xSize, zSize, rotation, shape = 1) {
  const color = hsl(0, 0, 0, 0.5);
  ...
  if (rotation) {
    ...
    points = points.map((p) =>
      vec3(p.x * size.x + pos.x, pos.y, p.z * size.z + pos.z)
    );
