...
function initHUD() {
  ...
  HUDButtons.push(
    new HUDButton("RADIO", vec3(0.63, 0.95), vec3(0.15, 0.08), 0, YELLOW, 0)
  );
  for (let i = 4; i--; ) {
    let c = hsl(0.1, 1, 0.5);
    let b = new HUDButton(
      i == 3 ? "OFF" : i + 1,
      vec3(0.73 + i * 0.07, 0.95),
      vec3(0.06),
      undefined,
      c
    );
...
function drawHUD() {
    ...
    for (let j = 2; j--; ) {
      ...
      for (let k = 2; k--; ) {
        for (let i = 0; i < text.length; i++) {
          ...
          const size2 = size + p * 0.05 * mainCanvasSize.y;
          ...
          for (let f = 2; f--; ) {
            const o = f * 0.01 * mainCanvasSize.y;
            context.fillStyle = hsl(0.15 + p / 9, 1, !f ? 0.75 + p * 0.25 : 0);
  ...
  if (startCountdownTimer.active() || startCountdown > 0) {
    ...
    if (gameOverTimer.isSet()) {
      const c = WHITE;
      const s1 = 0.04 * (1 - Math.abs(Math.sin(time * 2)));
      const s2 = 0.04 * (1 - Math.abs(Math.sin(time * 2 + PI / 2)));
      drawHUDText(
        "GAME",
        vec3(0.5, 0.1),
        0.1 + s1,
      ...
      const c =
        checkpointTimeLeft < 3 ? RED : checkpointTimeLeft < 10 ? YELLOW : WHITE;
  ...
  if (radioMusic >= 0) {
    let size = 0.034 + 0.002 * Math.sin(time * 4);
    drawHUDText(
      "𝅘𝅥𝅮 " + musicTrackNames[radioMusic],
      vec3(0.83, 0.89),
      size,
      WHITE,
      0.003,
  ...
  for (const b of HUDButtons) b.draw();
}
...
class HUDButton {
  ...
  draw() {
    ...
    if (this.musicTrack == radioMusic) {
      backgroundColor = hsl(radioMusic < 0 ? 0 : 0.1, 1, 0.5);
      color = WHITE;
      outlineColor = BLACK;
    }

    backgroundColor &&
      drawHUDRect(pos, this.size, backgroundColor, 0.005, outlineColor);

    pos.y += this.size.y * 0.05;
    drawHUDText(
      this.text,
      pos,
      this.size.y * 0.8,
...

    {
      ...
      if (this.onClick && mouseWasPressed(0))
        if (isOverlapping(p1, size, p2)) this.onClick();
    }
  }
}
...
function HUDstickToSides(pos) {
  pos = pos.copy();
  if (pos.x < 0.5) pos.x = (pos.x * mainCanvasSize.y) / mainCanvasSize.x;
  else pos.x = 1 - ((1 - pos.x) * mainCanvasSize.y) / mainCanvasSize.x;
  return pos;
}

///////////////////////////////////////////////////////////////////////////////

function drawHUDRect(
  pos,
  size,
  color = WHITE,
  lineWidth = 0,
  ...
  pos = pos.multiply(mainCanvasSize).subtract(size.scale(0.5));
...
function drawHUDText(
  ...
  if (stickToSides) pos = HUDstickToSides(pos);
