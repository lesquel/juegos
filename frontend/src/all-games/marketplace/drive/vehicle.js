...
function drawCars() {
  ...
  glPolygonOffset(80);
  glSetDepthTest(1, 0);
  for (let v of vehicleShadowList) pushShadow(...v);
  glRender();
...
class Vehicle {
  ...
  update() {
    ...
    if (this.velocity.z < this.targetSpeed) this.velocity.z += 0.5;
    else if (this.velocity.z > this.targetSpeed + 5) this.velocity.z -= 2;
    ...
    if (!trackInfo.pos || !trackInfo2.pos) return; // not visible
  ...
  draw() {
    ...
    if (this.isPlayer) {
      quadMesh.renderTile(
        m1.multiply(
          buildMatrix(vec3(0, bumperY - 80, bumperZ - 20), 0, vec3(80, 25))
        ),
        WHITE,
        getGenerativeTile(vec3(3, 0))
      );

      // top number
      quadMesh.renderTile(
        m1.multiply(
          buildMatrix(vec3(0, 220, -200), vec3(PI / 2 - 0.2, 0, 0), vec3(150))
        ),
        WHITE,
        getGenerativeTile(vec3(4, 0))
      );
    }
    ...
    for (let i = 2; i--; ) {
      let color = isBraking ? hsl(0, 1, 0.5) : hsl(0, 0.9, 0.3);
      cubeMesh.renderUnlit(
        m1.multiply(
          buildMatrix(
            vec3((i ? 1 : -1) * 180, bumperY - 25, bumperZ),
            0,
            vec3(40, 25, 20)
          )
        ),
    ...
    const wheelM1 = buildMatrix(
      0,
      vec3(this.pos.z / 500, this.wheelTurn, 0),
      wheelSize
    );
...
class PlayerVehicle extends Vehicle {
  ...
  update() {
    if (attractMode) {
      this.pos.z += this.velocity.x = min((this.velocity.x += 0.1), 20);
      return;
    }
    ...
    // player settings
    const forwardDamping = 0.998; // dampen player z speed
    const playerMaxSpeed = 200; // limit max player speed
    const playerTurnControl = 0.3; // player turning rate
    ...
    const elasticity = 0;
    1.2; // bounce elasticity (2 is full bounce, 1 is none)
    if (this.pos.y < playerTrackInfo.offset.y) {
      ...
      if (Math.abs(this.pos.x) > playerTrackInfo.width - this.collisionSize.x) {
        ...
        this.bumpTimer += this.velocity.z * rand(0.8, 1.2);
        if (this.bumpTimer > 200) {
          this.velocity.y += min(50, this.velocity.z) * 0.1 * rand(1, 2);
          this.bumpTimer = 0;
          sound_bump.play();
          //zzfx();
        }
      ...
      if (this.velocity.z < 10) this.velocity.z *= 0.95;
    ...
    if (onGround) {
      if (playerInput.y > 0)
        this.velocity.z +=
          playerInput.y *
          lerp(this.velocity.z / playerMaxSpeed, playerAccel, 0);
      else if (this.isBraking) this.velocity.z += playerInput.y * playerBrake;
    }
