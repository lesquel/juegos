...
function glPolygonOffset(offset = 50, scale = 1) {
  ...
  } else glContext.disable(gl_POLYGON_OFFSET_FILL);
}
...
function glPreRender() {
  ...
  debug && glContext.clearColor(0.4, 0.6, 0.9, 1);
  ...
  // set up the lights
  const initUniform4f = (name, x, y, z, w = 0) =>
    glContext.uniform4f(glUniform(name), x, y, z, w);
...
function glRender(transform = new DOMMatrix()) {
  ...
  const vertexData = glPositionData.subarray(
    0,
    glDrawCount * gl_INDICIES_PER_VERT
  );
