...
function gameStart() {
  ...
  cameraRot = vec3();
  vehicles = [];
  playerVehicle = new PlayerVehicle(2e3, hsl(0, 0.8, 0.5));
  vehicles.push(playerVehicle);

  for (let i = 10; i--; )
    vehicles.push(new Vehicle(5e3 * i + 3e3, hsl(rand(), 0.8, 0.5)));
}
...
function gameUpdateInternal() {
  if (attractMode) {
    ...
    if (vehicles.length < 10 && attractVehicleSpawnTimer-- < 0) {
      vehicles.push(
        new Vehicle(playerVehicle.pos.z - 1e3, hsl(rand(), 0.8, 0.5))
      );
