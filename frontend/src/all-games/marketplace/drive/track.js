...
function trackPreRender() {
  ...
  worldHeading += 0.00005 * cameraTrackInfo.offset.x * playerVehicle.velocity.z;

  if (attractMode) cameraPos.y = cameraTrackInfo.offset.y + 1e3;
  else cameraPos.y = cameraTrackInfo.offset.y + cameraPlayerOffset.y;

  cameraRot.x = cameraTrackInfo.pitch / 2;
  cameraPos.x = playerVehicle.pos.x * 0.7;

  // track settings
  lightDirection = vec3(0, 4, -1).rotateY(worldHeading).normalize();
  lightColor = hsl(0, 1, 1);
  ambientColor = hsl(0.7, 0.1, 0.2);
  fogColor = hsl(0.6, 1, 0.7);
  ...
  for (let i = sceneryDrawDistance; i--; ) {
    ...
    for (let i = 5; i--; ) {
      ...
      pushTrackSprite(
        p,
        vec3(s * m, s, s),
        hsl(random.float(0.1, 0.15), 1, 0.5),
        vec3(1, 1)
      );
    }

    // collidable sprites
    for (const sprite of trackSegment.sprites) sprite.draw(trackSegment);
  }
...
function pushTrackSprite(pos, scale, color, tilePos) {
  ...
  pushShadow(pos.add(vec3(0, offset)), scale.y, scale.y * 0.2);
  pushSprite(
    pos.add(vec3(0, offset + scale.y)),
    scale,
    color,
    getGenerativeTile(tilePos)
  );
}
...
class TrackSegment {
  constructor(segmentIndex, offset, width) {
    ...
    if (segmentIndex % checkpointTrackSegments < 5) checkpointLine = 1;
    {
      // setup colors
      const largeSegmentIndex = (segmentIndex / 6) | 0;
      const stripe = largeSegmentIndex % 2 ? 0.1 : 0;
      this.colorGround = hsl(
        0.083,
        0.2,
        0.7 + Math.cos((segmentIndex * 2) / PI) * 0.05
      );
      this.colorRoad = hsl(0, 0, stripe ? 0.6 : 0.55);
      if (checkpointLine) this.colorRoad = WHITE; // starting line
      this.colorLine = hsl(0, 0, 1, stripe ? 1 : 0);
    }

    // spawn sprites
    const addSprite = (...a) => this.sprites.push(new TrackSprite(...a));
    if (segmentIndex % checkpointTrackSegments == 0) {
      // checkpoint
...
function buildTrack() {
  ...
  for (let i = 0; i < trackEnd + 3e3 + drawDistance; ++i) {
    if (roadGenSectionDistance++ > roadGenSectionDistanceMax) {
      ...
      Math.min(
        1,
        (i * trackSegmentLength) / checkpointDistance / checkpointMaxDifficulty
      );

      // randomize road settings
      roadGenWidth = trackWidth; //*random.float(1-difficulty*.7, 3-2*difficulty);
      roadGenWaveFrequencyX = random.float(lerp(difficulty, 0.01, 0.03));
      roadGenWaveFrequencyY = random.float(lerp(difficulty, 0.01, 0.1));
      roadGenWaveScaleX =
        i > trackEnd ? 0 : random.float(lerp(difficulty, 0.2, 0.8));
