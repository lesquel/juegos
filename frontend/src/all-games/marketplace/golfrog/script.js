...
const screenToWorld = (pos) =>
  Vec.add(
    Vec.scale(Vec.subtract(pos, { x: 0.5, y: 0.5 }), camera.size),
    camera.pos
  );
const worldToScreen = (pos) =>
  Vec.add(Vec.scale(Vec.subtract(pos, camera.pos), camera.size ** -1), {
    x: 0.5,
    y: 0.5,
  });
...
const createStar = (pos) => ({
  ...
  shapes: [
    {
      x: [0, -0.07, -0.26, -0.12, -0.16, 0, 0.16, 0.12, 0.26, 0.07],
      y: [-0.5, -0.32, -0.3, -0.18, 0, -0.09, 0, -0.18, -0.3, -0.32],
      w: 0.1,
      color: 1,
      fill: true,
    },
...
const startHole = (h) => {
  ...
  if (h === 1) {
    ...
    player.pos = { x: -2, y: 1 };
    flag.pos = { x: 7.5, y: 0 };
    entities.push(createStar({ x: 0.5, y: 0.2 }));
    entities.push(createStar({ x: 2.5, y: 0.6 }));
    entities.push(createStar({ x: 4.5, y: -0.2 }));
    ground = (x) => 0.5 * Math.sin(x) + 0.5 * Math.sin(0.5 * x);
  } else if (h === 5) {
    ...
    entities.push(createStar({ x: 9, y: 0.5 }));
    entities.push(createCloud({ x: -1, y: -3 }));
    entities.push(createCloud({ x: 10, y: -4 }));
    ground = (x) => 0.7 * Math.sin(x) + 0.5 * Math.sin(0.5 * x - Math.PI);
  } else if (h === 6) {
    ...
    ground = (x) =>
      0.7 -
      3 * Math.exp(-0.5 * (x - 4.5) ** 2) -
      6 * Math.exp(-0.5 * (x - 10) ** 2);
  } else if (h === 7) {
    ...
    flag.pos = { x: 13.6, y: 0 };
    entities.push(createStar({ x: 5.24, y: -0.4 }));
    entities.push(createStar({ x: 9.4, y: -0.5 }));
    entities.push(createCloud({ x: 11, y: -5 }));
    ground = (x) => 0.7 - 0.2 * x ** 0.5 * Math.sin(1.5 * x);
  } else if (h === 9) {
    ...
    entities.push(createStar({ x: 5, y: -0.4 }));
    entities.push(createStar({ x: 9, y: -1 }));
    ...
    ground = (x) => -0.12 * x ** 2 + 0.1 * Math.sin(2 * x);
  } else if (h === 12) {
    ...
    ground = (x) => 0.8 * Math.sin(x) + 8 * Math.exp(-0.3 * (x - 3.5) ** 2);
  }
};
...
const draw = () => {
  ...
  for (const e of entities.filter((e) => e.shadow)) {
    ...
    ctx.fillStyle = "#000";
    ctx.globalAlpha = 0.2 * Math.exp(-d);
    ctx.fill();
    ctx.restore();
  }
  ...
  // Drag direction
  const drag = Vec.limit(
    Vec.subtract(input.dragEnd, input.dragStart),
    player.jump.limit
  );
  const showDrag = !state.won && player.grounded && Vec.length(drag) > 0.01;
  if (showDrag) {
    ctx.save();
    ctx.translate(player.pos.x, player.pos.y - 0.2);
    ctx.rotate(Vec.angle(drag));
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.lineTo(3 * Vec.length(drag) + 0.2, 0);
  ...
  if (player.diving) {
    ...
    for (let i = 0; i < n; i++) {
      ctx.save();
      ctx.translate(0.5 * (rand() - 0.5), 0.5 * (rand() - 0.5));
      ctx.beginPath();
      const start = Vec.add(
        Vec.add(player.pos, Vec.scale(Vec.normalize(player.vel), -0.3)),
        { x: 0, y: -0.3 }
      );
      const end = Vec.subtract(
        start,
        Vec.scale(
          Vec.normalize(player.vel),
          0.1 + 0.05 * Vec.length(player.vel)
        )
      );
  ...
  if (showDrag) {
    ...
    const dt = 0.5 / Vec.length(vel);
    let dist = 0.1;
    for (let i = 0; i < 10; i++) {
      pos = Vec.add(pos, Vec.scale(vel, dt));
      vel = Vec.scale(
        Vec.add(vel, { x: 0, y: player.gravity * dt }),
        player.damping ** dt
      );
      dist += Vec.length(vel) * dt;
      const r = 1 - Math.exp(-dist);
      ctx.beginPath();
      ctx.arc(pos.x, pos.y - 0.2, 0.1 * r, 0, 2 * Math.PI);
      ctx.globalAlpha = Math.max(0, -0.16 * dist * (dist - 5));
      ctx.fill();
    }
    ctx.restore();
  }
  ...
  if (location.hash === "#debug") {
    ...
    pre.innerText = JSON.stringify(
      dbg,
      (k, v) => (v.toFixed == null ? v : Number(v.toFixed(3))),
      2
    );
  }
...
const update = () => {
  ...
  player.grounded =
    player.pos.y >= ground(player.pos.x) && Vec.length(player.vel) * dt < 0.01;

  // Detect dive
  player.diving = !state.won && !player.grounded && input.primary;
  player.gravity = player.diving ? 30 : 10;
  player.physics.bounce = player.diving ? 0.9 : 0;
  ...
  if (!input.primary) {
    ...
    if (!state.won && player.grounded && Vec.length(drag) > 0.02) {
      state.jumps++;
  ...
  // Win condition
  if (!state.won && Vec.distance(player.pos, flag.pos) < 0.5) {
    const pos = Vec.add(flag.pos, { x: 0, y: -0.2 });
    for (let i = 0; i < 50; i++) {
      const vel = Vec.scale(
        Vec.rotate({ x: 0, y: -1 }, 1.5 * (Math.random() - 0.5)),
        5 * (Math.random() + 0.2)
      );
      entities.push({
        pos,
  ...
  // Flag animation
  flag.rot = 0.1 * Math.sin(1.5 * flag.age);

  // Star animation
  for (const e of entities.filter((e) => e.star)) {
    e.pos = Vec.add(e.star, { x: 0, y: 0.05 * Math.sin(2 * e.age) });
  }

  // Star collection
  for (const e of entities.filter((e) => e.star)) {
    if (Vec.distance(player.pos, e.star) < 0.5) {
      const pos = Vec.add(e.star, { x: 0, y: -0.2 });
      const n = 10;
      for (let i = 0; i < n; i++) {
        const vel = Vec.scale(
  ...
  for (const e of entities.filter((e) => e.physics)) {
    ...
    if (e.pos.y > ground(e.pos.x)) {
      ...
      const ref = Vec.add(
        e.vel,
        Vec.scale(normal, -2 * Vec.dot(e.vel, normal))
      );
      const a = Vec.angle(g);
      const rot = Vec.rotate(ref, -a);
      e.vel = Vec.rotate(
        { x: e.physics.friction ** dt * rot.x, y: e.physics.bounce * rot.y },
        a
      );
    }
  }
...
const resize = () => {
  const unit = 32;
  const size = Math.min(
    Math.floor(Math.min(window.innerWidth, window.innerHeight) / unit),
    24
  );
  ...
  overlay.style.fontSize = `${0.04 * canvas.width}px`;
};
